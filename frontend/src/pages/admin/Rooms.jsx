...
const AdminRooms = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      const [roomsRes, catsRes] = await Promise.all([
        ...
        axios.get(CAT_API_URL, { headers: { Authorization: `Bearer ${token}` } })
      ]);
  ...
  const filteredRooms = rooms.filter(room => 
    ...
    getCategoryName(room.roomCategoryId).toLowerCase().includes(searchTerm.toLowerCase()) ||
      ...
      <main className="flex-1 ml-64 p-12 min-h-screen">
        ...
        {loading ? (
          <div className="flex flex-col justify-center items-center h-96 space-y-4">
            ...
            <p className="text-xs uppercase tracking-widest text-luxury-gold animate-pulse">Scanning Inventory...</p>
          </div>
        ) : (
          <motion.div 
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-luxury-dark/5 dark:bg-white/5 border-b border-black/5 dark:border-white/5">
                    <th className="px-8 py-6 text-[10px] uppercase tracking-widest font-bold text-luxury-charcoal/60 dark:text-white/60">Room No.</th>
                    ...
                    <th className="px-8 py-6 text-[10px] uppercase tracking-widest font-bold text-luxury-charcoal/60 dark:text-white/60">Rate / Night</th>
                    <th className="px-8 py-6 text-[10px] uppercase tracking-widest font-bold text-luxury-charcoal/60 dark:text-white/60">Status</th>
                    <th className="px-8 py-6 text-[10px] uppercase tracking-widest font-bold text-luxury-charcoal/60 dark:text-white/60 text-right">Actions</th>
                  </tr>
                </thead>
                    ...
                    {filteredRooms.length > 0 ? filteredRooms.map((room) => (
                      ...
                      >
                        <td className="px-8 py-6">
                          <span className="text-lg font-serif text-luxury-charcoal dark:text-white font-bold">{room.roomNumber}</span>
                        </td>
                        <td className="px-8 py-6">
                          <span className="text-xs font-bold uppercase tracking-widest text-luxury-gold">{getCategoryName(room.roomCategoryId)}</span>
                        </td>
                        <td className="px-8 py-6">
                          <span className="text-sm font-medium text-luxury-charcoal dark:text-white">${room.ratePerNight}</span>
                        </td>
                        <td className="px-8 py-6">
                          <span className={`px-3 py-1 rounded-full text-[8px] font-bold uppercase tracking-widest ${
                            ...
                            room.status === 'booked' ? 'bg-luxury-gold/10 text-luxury-gold' :
                      ...
                      <tr>
                        <td colSpan="5" className="px-8 py-20 text-center">
                          <div className="flex flex-col items-center space-y-4">
                            ...
                            <p className="text-luxury-charcoal/30 dark:text-white/30 text-sm font-medium">No rooms found in the vault.</p>
                          </div>
                        </td>
                      </tr>
                    )}
            ...
            >
              <div className="flex justify-between items-center p-8 border-b border-black/5 dark:border-white/5 bg-white dark:bg-luxury-charcoal text-luxury-charcoal dark:text-white">
                <div>
                  <h2 className="text-3xl font-serif mb-1">
                    {editingRoom ? 'Update Room Strategy' : 'Initialize New Room'}
                  </h2>
                  <p className="text-[10px] text-luxury-gold uppercase tracking-[0.3em] font-bold">Room Configuration Terminal</p>
                </div>
                <button 
                  onClick={handleCloseModal}
              ...
              <form onSubmit={handleSubmit} className="p-8 space-y-8 overflow-y-auto flex-1 custom-scrollbar">
                ...
                <div className="space-y-4">
                  <label className="text-[10px] uppercase tracking-widest font-bold text-luxury-gold">Visual Assets (Upload 3 Images)</label>
                    ...
                    {[1, 2, 3].map((num) => (
                      <div key={num} className="space-y-3">
                        <div className="relative group">
                          ...
                          >
                            {previews[`image${num}`] ? (
                              <img src={previews[`image${num}`]} alt={`Preview ${num}`} className="w-full h-full object-cover" />
                            ) : (
                              <div className="flex flex-col items-center space-y-2 opacity-40">
                                <ImageIcon size={24} />
                                <span className="text-[8px] font-bold uppercase">Image {num}</span>
                              </div>
                            )}
                          </label>
                        </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-6">
                    <div className="space-y-2">
                      <label className="text-[10px] uppercase tracking-widest font-bold text-luxury-gold">Room Number</label>
                      <input 
                        type="text" 
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] uppercase tracking-widest font-bold text-luxury-gold">Category Assignment</label>
                      <select 
                        value={formData.roomCategoryId}
                        onChange={(e) => setFormData({...formData, roomCategoryId: e.target.value})}
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] uppercase tracking-widest font-bold text-luxury-gold">Rate Per Night ($)</label>
                      <input 
                        type="number" 
                  ...
                  <div className="space-y-6">
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <label className="text-[10px] uppercase tracking-widest font-bold text-luxury-gold">Capacity (Guests)</label>
                        <input 
                          type="number" 
                      ...
                      <div className="space-y-2">
                        <label className="text-[10px) uppercase tracking-widest font-bold text-luxury-gold">Status</label>
                        <select 
                    ...
                    <div className="space-y-2">
                      <label className="text-[10px] uppercase tracking-widest font-bold text-luxury-gold">Bed Configuration</label>
                      <input 
                        type="text" 
                    ...
                    <div className="flex items-center space-x-3 pt-4">
                      <div 
                        onClick={() => setFormData({...formData, ac: !formData.ac})}
                      ...
                      >
                        <div className={`absolute top-1 left-1 w-4 h-4 bg-white rounded-full transition-all duration-300 ${formData.ac ? 'translate-x-6' : ''}`}></div>
                      </div>
                      <span className="text-[10px] uppercase tracking-widest font-bold text-luxury-charcoal/60 dark:text-white/60">Air Conditioned</span>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-6">
                  <div className="space-y-2">
                    <label className="text-[10px] uppercase tracking-widest font-bold text-luxury-gold">Amenities (Comma separated)</label>
                    <textarea 
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] uppercase tracking-widest font-bold text-luxury-gold">Internal Administrative Notes</label>
                    <textarea 
                ...
                <div className="pt-8 border-t border-black/5 dark:border-white/5">
                  ...
                  >
                    {isSubmitting ? 'Syncing with mainframe...' : (editingRoom ? 'Commit Changes' : 'Initialize Room Entry')}
                  </button>
                </div>
              </form>
            </motion.div>
            ...
            >
              ...
              <div className="w-full md:w-[45%] h-[40vh] md:h-full relative bg-luxury-dark border-r border-white/5 overflow-hidden">
                <div className="absolute inset-0 flex flex-col">
                  {[selectedRoom.image1, selectedRoom.image2, selectedRoom.image3].filter(Boolean).length > 0 ? (
                    [selectedRoom.image1, selectedRoom.image2, selectedRoom.image3].map((img, i) => (
                      img && (
                        <div key={i} className="flex-1 relative overflow-hidden group border-b border-white/5">
                          <img 
                            src={`${BASE_URL}${img}`} 
                ...
                <div className="absolute bottom-12 left-12 z-10 hidden md:block">
                  <span className="text-[12px] uppercase tracking-[0.6em] text-luxury-gold font-black mb-4 block">Elite Residence</span>
                  <h2 className="text-8xl font-serif text-white tracking-tighter leading-none mb-4">{selectedRoom.roomNumber}</h2>
                ...
                <div className="p-8 md:p-20 overflow-y-auto custom-scrollbar flex-1">
                  <header className="mb-16">
                    <div className="flex items-center space-x-4 text-luxury-gold mb-6">
                      ...
                      <span className="text-[10px] uppercase tracking-[0.5em] font-black">Room Specification Profile</span>
                    </div>
                    <h1 className="text-4xl md:text-6xl font-serif text-luxury-charcoal dark:text-white tracking-tight mb-2">Detailed Overview</h1>
                    <p className="text-luxury-charcoal/40 dark:text-white/40 font-medium italic">Internal System Record ID: {selectedRoom.id}</p>
                  </header>
                    ...
                    <div className="space-y-12">
                      <section>
                        <p className="text-[10px] uppercase tracking-widest text-luxury-gold font-black mb-4 flex items-center">
                          <Info size={14} className="mr-2" /> Strategic Info
                        </p>
                        <div className="space-y-8">
                          <div>
                            <p className="text-[9px] uppercase tracking-widest text-luxury-charcoal/40 dark:text-white/40 font-bold mb-1">Asset Category</p>
                            <p className="text-3xl font-serif text-luxury-charcoal dark:text-white">{getCategoryName(selectedRoom.roomCategoryId)}</p>
                          </div>
                          <div>
                            <p className="text-[9px] uppercase tracking-widest text-luxury-charcoal/40 dark:text-white/40 font-bold mb-1">Nightly Valuation</p>
                            <p className="text-3xl font-serif text-luxury-charcoal dark:text-white">${selectedRoom.ratePerNight} <span className="text-sm font-sans text-luxury-charcoal/30 uppercase tracking-widest">USD</span></p>
                          </div>
                        </div>
                      </section>

                      <section>
                        <p className="text-[10px] uppercase tracking-widest text-luxury-gold font-black mb-4">Operational Status</p>
                        <div className="flex items-center space-x-4">
                          <span className={`px-6 py-2 rounded-sm text-[10px] font-black uppercase tracking-[0.3em] ${
                            selectedRoom.status === 'available' ? 'bg-green-500/10 text-green-500 border border-green-500/20' :
                            selectedRoom.status === 'booked' ? 'bg-luxury-gold/10 text-luxury-gold border border-luxury-gold/20' :
                    ...
                    <div className="space-y-12">
                      <section>
                        ...
                        <div className="grid grid-cols-2 gap-8">
                          <div>
                            <p className="text-[9px] uppercase tracking-widest text-luxury-charcoal/40 dark:text-white/40 font-bold mb-1">Capacity</p>
                            <p className="text-xl font-serif text-luxury-charcoal dark:text-white">{selectedRoom.capacity} PAX</p>
                          </div>
                          <div>
                            <p className="text-[9px] uppercase tracking-widest text-luxury-charcoal/40 dark:text-white/40 font-bold mb-1">Bed Type</p>
                            <p className="text-xl font-serif text-luxury-charcoal dark:text-white capitalize">{selectedRoom.bedType || 'N/A'}</p>
                          </div>
                          <div className="col-span-2">
                            <p className="text-[9px] uppercase tracking-widest text-luxury-charcoal/40 dark:text-white/40 font-bold mb-1">Climate Control</p>
                            <div className="flex items-center space-x-2 text-luxury-charcoal dark:text-white">
                              {selectedRoom.ac ? <Check className="text-green-500" size={16} /> : <AlertCircle className="text-red-400" size={16} />}
                              <span className="text-sm font-bold uppercase tracking-widest">{selectedRoom.ac ? 'Full HVAC System' : 'Natural Ventilation'}</span>
                            </div>
                          </div>
                        </div>
                      </section>
                  ...
                  <div className="space-y-16 border-t border-black/5 dark:border-white/5 pt-16">
                    {selectedRoom.amenities && (
                      <div>
                        <p className="text-[10px] uppercase tracking-widest text-luxury-gold font-black mb-6">Signature Amenities</p>
                        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                          {selectedRoom.amenities.split(',').map((amenity, i) => (
                            <div key={i} className="flex items-center space-x-3 text-luxury-charcoal/60 dark:text-white/60">
                              <div className="w-1 h-1 bg-luxury-gold rounded-full"></div>
                              <span className="text-xs uppercase tracking-widest font-bold">{amenity.trim()}</span>
                            </div>
                          ))}
                        </div>
                      </div>
                    ...
                    {selectedRoom.notes && (
                      <div className="bg-luxury-dark/5 dark:bg-white/5 p-12 rounded-sm border border-black/5 dark:border-white/5">
                        <p className="text-[10px] uppercase tracking-widest text-luxury-gold font-black mb-6">Management Directives</p>
                  ...
                  <div className="mt-24 pt-10 border-t border-black/5 dark:border-white/5 flex flex-col md:flex-row justify-between items-center gap-8">
                    <div className="hidden lg:block">
                      <p className="text-[10px] uppercase tracking-widest text-luxury-charcoal/30 dark:text-white/30 font-bold italic">Generated by Ocean View Resort Admin Authority System v2.0</p>
                    </div>
